...
class ProjectResearcher(BaseModel):
    ...
    competition: str = Field(description="Competition and competitors for the organization")
...
class ResumeWriter(BaseModel):
    resume: str = Field(description="modified resume text with <modified> tags around changed bullet points")
    explanation: str = Field(description="rationale behind the modifications")


class CoverLetterWriter(BaseModel):
    cover_letter: str = Field(description="enhanced cover letter text with <modified> tags around changed paragraphs")
    explanation: str = Field(description="rationale behind the cover letter modifications")
